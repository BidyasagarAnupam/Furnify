...
import { setCategory, setEditSubCategory, setSubCategory } from '../../../slices/categorySlice';
...
const AdminDashboard = () => {
  ...
  const handleEditSubCategory = (item) => {
    ...
    navigate(`/dashboard/adminDashboard/editsubCategory/${true}`)
  }

  const deleteModalHandler = (subCid) => { 
    setSelectedSubCategory(subCid);
    onOpen()
  }

  const subCategoryDeleteHandler = async (onClose) => {
     await deleteSubCategory(selectedSubCategory, token)
    setIsDeleteSubCategory(!isDeleteSubCategory)
    onClose()
  }
    ...
    <div className='flex flex-col items-start gap-8'>
              ...
              >
                <div className='flex justify-center items-center gap-4'>
                  <div className='flex flex-col items-center justify-center gap-2 hover:cursor-pointer  hover:font-semibold transition-all duration-100'>
                    <div onClick={() => navigate(`/dashboard/adminDashboard/addsubCategory/${selectedCategory}`)} className='flex justify-center items-center w-[70px] aspect-square border-2 border-dashed rounded-md bg-[#EAF8FF] border-[#2086BF]'>
                      <FaSquarePlus className='text-xl text-[#16597F]' />
                    </div>
                    <p className='text-sm'>Add Category</p>
                  </div>
                  <div className='flex flex-col items-center justify-center gap-2 hover:cursor-pointer  hover:font-semibold transition-all duration-100'>
                    <div onClick={() => navigate(`/dashboard/adminDashboard/addsubCategory/${selectedCategory}`)} className='flex justify-center items-center w-[70px] aspect-square border-2 border-dashed rounded-md bg-[#D3F4EF] border-[#1A9882]'>
                      <FaPenClip className='text-xl text-[#1A9882]' />
                    </div>
                    <p className='text-sm'>Edit Category</p>
                  </div>
                  <div className='flex flex-col items-center justify-center gap-2 hover:cursor-pointer  hover:font-semibold transition-all duration-100'>
                    <div onClick={() => navigate(`/dashboard/adminDashboard/addsubCategory/${selectedCategory}`)} className='flex justify-center items-center w-[70px] aspect-square border-2 border-dashed rounded-md bg-[#FBD8DB] border-[#EB3D4D]'>
            ...
            subCategoryLoading ?
              ...
              (
                <div className='grid grid-cols-4 gap-3 border-2 border-dotted rounded-md bg-[#F6F5F2] border-[#BBAB8C] p-3 '>
                      ...
                      <div className='flex flex-col items-center justify-center gap-2 hover:cursor-pointer  hover:font-semibold transition-all duration-100'>
                        <div className=' group relative'>
                          ...
                          <div className=' gap-2 absolute top-0 w-full h-full bg-[#4c505177] rounded-md hidden group-hover:flex items-center justify-center !opacity-100 transition-all duration-400 ease-in'>
                            ...
                            <MdDelete className='text-2xl  text-red-600' onClick={() => deleteModalHandler(item._id)} />
                          </div>
                        </div>
                        <p className="text-sm"
                        >{item.name}</p>
                  ...
                  <div className='flex flex-col items-center justify-center gap-2 hover:cursor-pointer  hover:font-semibold transition-all duration-100'>
                    <div onClick={() => navigate(`/dashboard/adminDashboard/addsubCategory/${selectedCategory}`)} className='flex justify-center items-center w-[100px] aspect-square border-2 border-dashed rounded-md bg-[#EAF8FF] border-[#2086BF]'>
      ...
      <Modal backdrop='blur' isOpen={isOpen} onClose={onClose}>
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1 border-b-2">🚨Are you sure to delete this subcategory?</ModalHeader>
              <ModalBody>
                <p>
                  All Products will be deleted which are inside this subcategory!
                </p>
              </ModalBody>
              <ModalFooter>
                ...
                <Button color="primary" onPress={() => subCategoryDeleteHandler(onClose)}>
                  Delete
                </Button>
              </ModalFooter>
            </>
