...
import { resetCart, setStep } from "../../slices/cartSlice";

const {PRODUCT_PAYMENT_API, PRODUCT_VERIFY_API, SEND_PAYMENT_SUCCESS_EMAIL_API} = customerEndpoints;
...
export async function buyProduct(token, products, totalDiscountedPrice,addressId, userDetails, navigate, dispatch) {
    ...
    try{
        ...
        const orderResponse = await apiConnector("POST", PRODUCT_PAYMENT_API, 
            {
                products,
                totalAmount: totalDiscountedPrice + 500,
            },
                                {
